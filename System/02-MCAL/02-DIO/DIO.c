		...
		if(PortId < NUM_OF_PORTS)
		{
			...
			if (PinDir == 0) /* Input */
			{
				...
				if (PinNumber < 8)
				{
					...
					/* Clear the 4-bit segment before write */
					GPIOx->CRH &= ~( (0x0F)<< (4*(PinNumber-8)) );
					GPIOx->CRH |= (PinDir&0x03)<<(4*(PinNumber-8)) /* 1st bit */ |
						(PinDir&0b00000100)<<(4*(PinNumber-8)+1) | /* CNF1 4th bit */
						(DIO_ConfigParam[Loop].OutputType&0x01)<<(4*(PinNumber-8)+2);/*CNF0 3rd*/
				}
			}
		} else {
			/* Invalid port */
